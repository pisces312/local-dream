#!/usr/bin/env bash
# Local Dream Build Script (Git Bash / WSL / Linux)
# Usage: ./build-local.sh [debug|release] [basic|filter] [--rebuild-native] [--skip-gradle] [-d <device>]
...
#
# Examples:
#   ./build-local.sh                           # Build release, basic flavor
#   ./build-local.sh debug                     # Build debug, basic flavor
#   ./build-local.sh release filter            # Build release, filter flavor
#   ./build-local.sh debug -d 192.168.1.5:5555 # Build debug, install to device
#   ./build-local.sh release --rebuild-native  # Rebuild C++ backend then build APK
...
# ─── Resolve project directory (works in Git Bash on Windows) ───
SCRIPT_PATH="$(cd "$(dirname "$0")" && pwd)"
# If MSYS/Git Bash, convert to Windows-style for Gradle
if [[ "$OSTYPE" == "msys" || "$OSTYPE" == "win32" ]]; then
    # /d/3rd-party-projects/local-dream → D:\3rd-party-projects\local-dream
    PROJECT_DIR="$(cygpath -w "$SCRIPT_PATH")"
    PROJECT_DIR_UNIX="$SCRIPT_PATH"
else
    PROJECT_DIR="$SCRIPT_PATH"
    PROJECT_DIR_UNIX="$SCRIPT_PATH"
fi

APP_DIR="$PROJECT_DIR_UNIX/app"
...
# Use gradlew.bat on Windows/MSYS, gradlew on Linux/macOS
if [[ "$OSTYPE" == "msys" || "$OSTYPE" == "win32" ]]; then
    cmd //c "gradlew.bat $GRADLE_TASK"
else
    ./gradlew "$GRADLE_TASK"
fi
...
SOURCE_APK=""
# Try naming patterns (Gradle output filename varies by AGP version)
for candidate in \
    "$BUILD_DIR/LocalDream_armv8a_${VERSION}.apk" \
    "$BUILD_DIR/app-${FLAVOR}-${BUILD_TYPE}.apk"; do
    if [[ -f "$candidate" ]]; then
...
# Fallback: search for unsigned APK (release) or any APK
...
if [[ "$BUILD_TYPE" == "release" ]]; then
    ...
    KEY_ALIAS_VAL="${KEY_ALIAS:-pisces312}"

    if [[ -n "$KEYSTORE" && -n "$KEYSTORE_PASS" ]]; then
        # Resolve build-tools path
        if [[ "$OSTYPE" == "msys" || "$OSTYPE" == "win32" ]]; then
            BUILD_TOOLS="$(cygpath -u "${ANDROID_SDK_ROOT:-D:/dev/android_sdk}")/build-tools/34.0.0"
        else
            BUILD_TOOLS="${ANDROID_SDK_ROOT:-/opt/android_sdk}/build-tools/34.0.0"
        fi

        if [[ ! -f "$BUILD_TOOLS/lib/apksigner.jar" ]]; then
            echo "WARNING: apksigner.jar not found at $BUILD_TOOLS"
            echo "  Copying unsigned APK instead"
            OUTPUT_APK="$PROJECT_DIR_UNIX/LocalDream_armv8a_${VERSION}-${FLAVOR}-unsigned.apk"
            cp -f "$SOURCE_APK" "$OUTPUT_APK"
        else
            ALIGNED_APK="$BUILD_DIR/release-aligned.apk"
            OUTPUT_APK="$PROJECT_DIR_UNIX/LocalDream_armv8a_${VERSION}-${FLAVOR}-signed.apk"
    ...
    OUTPUT_APK="$PROJECT_DIR_UNIX/LocalDream_armv8a_${VERSION}-${FLAVOR}-debug.apk"
    cp -f "$SOURCE_APK" "$OUTPUT_APK"
fi
