...
fun ModelsStorageDialog(
    onDismiss: () -> Unit,
    onPathChanged: (complete: Boolean) -> Unit,
) {
    ...
    var migrationIncomplete by remember { mutableStateOf(false) }
    ...
    val dirPicker = rememberLauncherForActivityResult(
        ...
        if (uri != null) {
            ...
            if (fsPath != null) {
                if (isPublicStorageDir(fsPath)) {
                    // Downloads and cleanup write/delete inside the models
                    // dir; public locations (Download, DCIM, …) must never
                    // become one.
                    Toast.makeText(
                        context,
                        context.getString(R.string.models_storage_path_not_allowed),
                        Toast.LENGTH_LONG,
                    ).show()
                } else {
            ...
            TextButton(
                onClick = {
                    ...
                    scope.launch {
                        ...
                        // Refuse to migrate while a download/extract may be
                        // writing into either directory or a backend process
                        // holds model files open — a rename across those would
                        // strand the transfer (old location) or corrupt it.
                        ...
                        val backendActive = BackendService.backendState.value.let { state ->
                            state is BackendService.BackendState.Starting ||
                                state is BackendService.BackendState.Running
                        }
                        migrationStatus = when {
                            downloadActive -> context.getString(
                                R.string.models_storage_download_active,
                            )
                            backendActive -> context.getString(
                                R.string.models_storage_backend_active,
                            )
                            else -> null
                        }
                        if (migrationStatus != null) return@launch

                        // getModelsDir() silently falls back to internal
                        // storage for an unusable custom path; checking both
                        // paths explicitly stops a migration that would
                        // otherwise move/delete the wrong directory (e.g. old
                        // path on an unmounted SD card).
                        for (path in listOf(oldPath, newPath)) {
                            if (path != null && !Model.isCustomModelsPathUsable(path)) {
                                migrationStatus = context.getString(
                                    R.string.models_storage_path_unusable,
                                    path,
                                )
                                return@launch
                            }
                        ...
                        try {
                                ...
                                if (files != null && files.isNotEmpty()) {
                                    ...
                                    val complete = withContext(Dispatchers.IO) {
                                        ...
                                        for (f in files) {
                                            ...
                                            // Only drop the source once the copy
                                            // is verifiably there. A migration
                                            // killed mid-copy then leaves one
                                            // copy in each location instead of
                                            // losing the model outright.
                                            if (!dest.exists() || sizeOf(f) != sizeOf(dest)) {
                                                allMigrated = false
                                                continue
                                            }
                                            f.deleteRecursively()
                            ...
                            // Re-read the storage path so the upscaler list
                            // judges isDownloaded against the new location.
                            UpscalerRepository.getInstance(context).refreshStoragePath()

                            migrationStatus = if (migrationIncomplete) {
                                context.getString(
                                    R.string.models_storage_migrate_incomplete,
                                )
                            } else {
                                context.getString(
                            ...
                            onPathChanged(!migrationIncomplete)
                        } catch (e: Exception) {
                            migrationStatus = context.getString(
...
/**
 * Public locations on the primary volume must never become the models dir:
 * downloads, cleanup and migration write/delete inside it, and entries there
 * are the user's own files, not app-managed data.
 */
private fun isPublicStorageDir(path: String): Boolean {
...
/** Recursive byte size, used to verify a copied model before its source is dropped. */
